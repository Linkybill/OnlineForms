...
import * as strings from "NovaFormsAppCustomizerOnlineApplicationCustomizerStrings";
import { ActiveListFieldNames } from "../formTemplateListActionsOnline/Constants";
...
export default class NovaFormsAppCustomizerApplicationCustomizer extends BaseApplicationCustomizer<INovaFormsAppCustomizerApplicationCustomizerProperties> {
  ...
  public async onInit(): Promise<void> {
    ...
    const isInSubFolder = folderParam && currentListTitle && folderParam.toLowerCase().includes(`/${currentListTitle.toLowerCase()}/`);

    if (currentListTitle && isInSubFolder) {
      const id = queryParams.get("ID");
      if (id) {
        try {
          const requestUrl = `${this.context.pageContext.web.absoluteUrl}/_api/web/lists/getbytitle('${currentListTitle}')/items(${id})`;
          ...
          var formIdentifier = formItem[ActiveListFieldNames.formInstanceIdentifier] ?? formItem["FileLeafRef"];
          var url = formWebUrl + "/sitePages/formInstance.aspx?openInPanel=1&formId=" + formIdentifier;
