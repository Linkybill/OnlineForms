              ...
              {loading === false && formViewModel !== undefined && formViewModel.formTemplate.editorModel !== undefined && (
                <>
                  <FormConfigurationContextProvider>
                    <PermissionContextProvider listItemId={formViewModel === undefined ? undefined : formViewModel.formContent.listItem.ID}>
                      <EditorContextProvider editorModel={{ ...formViewModel.formTemplate.editorModel, customFieldDefinitions: [...formViewModel.formTemplate.editorModel.customFieldDefinitions] }} isInEditMode={false}>
                        <EditorContextConsumer>
                          {(editorContext) => {
                            return (
                              <>
                                <FormFileContextProvider listItemId={formViewModel.formContent.listItem.ID}>
                                          ...
                                          <FormContentLoadingContextProvider contentIsLoading={loading}>
                                            <ListItemContextProvider
                                              ...
                                              listItemHasConflictingChanges={async () => {
                                                if (currentListItemEtag.current === undefined) {
                                                  return false;
                                                }
                                                try {
                                                  var itemResult = await sp.web.lists
                                                    .getByTitle(ListNames.aktiveFormsListName)
                                                    .items.getById(currentItemId.current)
                                                    .update({ FormAction: "E" }, currentListItemEtag.current);
                                                  currentListItemEtag.current = itemResult.data["odata.etag"];
                                                  return false;
                                                } catch (e) {
                                                  return true;
                                                }
                                              ...
                                              onListItemSave={async (changedItem, filesToUpload: FileWithKey[], fileNamesToDelete: string[], filename: string): Promise<ListItem> => {
                                                ...
                                                if (changedItem.ID === undefined) {
                                                  try {
                                                    ...
                                                    const newItemFromSharePoint = await sp.web.lists.getByTitle(ListNames.aktiveFormsListName).items.getById(item.ID).get();
                                                    currentListItemEtag.current = newItemFromSharePoint["odata.etag"];
                                                    currentItemId.current = item.ID;
                                                    fileContext.resetFilesBeingUploaded();
                                                    setFormViewModel((old) => {
                                                      return { ...old, formContent: { ...old.formContent, listItem: item, etag: newItemFromSharePoint["odata.etag"] } };
                                                    });
                                                    return item;
                                                  } catch (e) {
